...
describe('admonitions', () => {
    ...
    it('supports the legacy space syntax', async () => {
        const md = ':::note Title\nContent\n:::';
        const result = await markdownToHtml(md);
        expect(result.toString()).toContain('admonition-note');
    ...
    it('renders caution admonitions', async () => {
        const md = ':::caution[Critical]\nDangerous operation!\n:::';
        const result = await markdownToHtml(md);
        expect(result.toString()).toContain('admonition-caution');
    });

    it('renders important admonitions', async () => {
        const md = ':::important[Information]\nUseful info\n:::';
        const result = await markdownToHtml(md);
        expect(result.toString()).toContain('admonition-important');
    });
...
describe('type links', () => {
    ...
    it('does not link unknown types', async () => {
        ...
        const result = await markdownToHtml(md, ['MyComponent']);
        expect(result.toString()).not.toContain('href="#/type/UnknownType"');
    ...
    it('does not link types inside code blocks', async () => {
        ...
        const result = await markdownToHtml(md, ['MyComponent']);
        expect(result.toString()).not.toContain('href="#/type/MyComponent"');
    });
